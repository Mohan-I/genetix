...
import { Dna, FlaskConical, Beaker, Info, ChevronRight, Sparkles, RefreshCw, AlertCircle, Thermometer } from 'lucide-react';
...
import { calculateBloodTypeProbabilities, calculateEyeColorProbabilities, calculatePathologyRisks, checkRhIncompatibility, predictPregnancyRisk } from './lib/geneticEngine';
...
export default function App() {
  ...
  const rhRisk = useMemo(() => checkRhIncompatibility(p1.bloodType, p2.bloodType), [p1.bloodType, p2.bloodType]);
  const pregnancyRisk = useMemo(() => predictPregnancyRisk(p1.maternalHealth!), [p1.maternalHealth]);
          ...
          <AnimatePresence mode="wait">
            {!showResults ? (
              ...
              >
                ...
                {/* Maternal Health Analysis & Rh Note */}
                <div className="grid md:grid-cols-2 border-b border-white/10">
                   <div className="p-8 space-y-6">
                      ...
                      <div className="flex items-center gap-4">
                        <div className={`px-4 py-2 text-xs font-mono border ${pregnancyRisk.status === 'HIGH' ? 'bg-red-500/10 border-red-500/30 text-red-500' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500'}`}>
                           STATUS: {pregnancyRisk.status} RISK
                        </div>
                        <div className="text-[10px] text-white/40 font-mono italic">ACCURACY: 91%</div>
                      </div>
                      <ul className="space-y-2">
                        {pregnancyRisk.notes.map((note, nidx) => (
                          <li key={nidx} className="text-[11px] text-white/60 flex gap-2">
                   ...
                   <div className="p-8 border-l border-white/10 space-y-6 bg-red-500/[0.02]">
                      <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                        <Thermometer className="w-3 h-3 text-blue-500" />
                        Blood Group Compatibility
                      </h3>
                      <div className={`p-4 border ${rhRisk.isAtRisk ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                         <p className={`text-[11px] font-medium leading-relaxed ${rhRisk.isAtRisk ? 'text-red-400' : 'text-emerald-400'}`}>
                           {rhRisk.message}
                         </p>
                      </div>
                      {rhRisk.isAtRisk && (
                        <div className="text-[10px] text-white/30 italic">
                          * Clinical monitoring for Rh antibodies is mandatory for Rh- mothers.
                        </div>
                      )}
                   </div>
                </div>
